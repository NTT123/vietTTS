...
class MelFilter:
  ...
  def __init__(self, sample_rate: int, n_fft: int, window_length, hop_length, n_mels: int, fmin=0.0, fmax=8000):
  ...
  def __call__(self, y: ndarray) -> ndarray:
    ...
    spec = batched_stft(y, self.n_fft, self.hop_length, self.window_length, 'hann', False, 'reflect')
    mag = jnp.sqrt(jnp.square(spec.real) + jnp.square(spec.imag) + 1e-9)
    mel = jnp.einsum('ms,sfn->nfm', self.melfb, mag)
    cond = jnp.log(mel + 1e-3)
    return cond
