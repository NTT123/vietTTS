...
lr_scheduler = optax.warmup_exponential_decay_schedule(
    0.0,
    FLAGS.learning_rate * num_devices,
    4_000,
    50_000,
    0.5,
    0,
...
"""
    ...
    tr = tqdm(
        range(last_step + spu, FLAGS.num_training_steps + spu, spu),
        desc="training",
    ...
    for step in tr:
        ...
        if step % 1000 == 0:
            ...
            plt.subplot(3, 1, 2)
            plt.imshow(
                gt_mel.T, origin="lower", aspect="auto", vmin=min_value, vmax=max_value
            )
            plt.subplot(3, 1, 3)
            ...
            with open(ckpt_fn, "wb") as f:
                params_, aux_, rng_, optim_state_ = jax.tree_map(
                    lambda x: jax.device_get(x[0]), (params, aux, rng, optim_state)
                )
                pickle.dump(
                    {
                        "step": step,
